...
export const useGetUserTransactions = (user: User) => {
  return useQuery({
    queryFn: user
      ? async () => {
          const transactionResponse = await parseApiResponse(
            honoClient.api.transactions['get-user-transactions'].$get({
              query: {
                id: user.id,
              },
            }),
  ...
  return useMutation({
    mutationFn: async (transactionPayload: PartialTransactionType) => {
      const response = await parseApiResponse(
        honoClient.api.transactions['create-user-transaction'].$post({
      ...
      if (response.status === RESPONSE_STATUS.SUCCESS) {
        toast({
          ...
          description: `Transaction created for amount worth ${parseFloat(transactionPayload.amount).toFixed(2)}.`,
        });
      } else {
